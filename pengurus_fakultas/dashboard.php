...
$query_my_requests = "SELECT item_requests.*, items.nama_barang 
                      FROM item_requests 
                      JOIN items ON item_requests.item_id = items.id 
                      WHERE item_requests.user_id = '$user_id'
                      ORDER BY item_requests.created_at DESC";
$result_my_requests = mysqli_query($conn, $query_my_requests);
...
<div id="user-modal"
    ...
    <div id="main-app-section" class="min-h-screen bg-[#F9FAFB] flex relative">
        ...
        <div>
            ...
            <nav class="space-y-2">
                <p class="px-4 text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Menu Staff Fakultas</p>
                ...
                <button onclick="switchAdminTab('persetujuan')" id="admin-btn-persetujuan"
                    ...
                    <i data-lucide="file-check-2" class="w-5 h-5"></i> Persetujuan
                    <?php if ($count_pending > 0): ?>
                        <span
                            class="ml-auto bg-red-100 text-red-600 px-2 py-0.5 rounded-full text-xs font-bold"><?= $count_pending ?></span>
                    <?php endif; ?>
                </button>
                <button onclick="switchAdminTab('inventaris')" id="admin-btn-inventaris"
                    class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium text-slate-500 hover:bg-slate-50 hover:text-luxury-secondary transition-all">
                    <i data-lucide="boxes" class="w-5 h-5"></i> Inventaris
                </button>
                <button onclick="switchAdminTab('users')" id="admin-btn-users"
                    class="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium text-slate-500 hover:bg-slate-50 hover:text-luxury-secondary transition-all">
    ...
    <div id="request-modal" class="fixed inset-0 z-[90] flex items-center justify-center bg-luxury-secondary/60 backdrop-blur-sm hidden px-4 overflow-y-auto py-4">
        <div class="bg-white rounded-[2rem] shadow-2xl animate-popup max-w-lg w-full relative overflow-hidden ring-1 ring-black/5">
            <div class="bg-luxury-primary px-8 py-6 flex justify-between items-center text-white">
                <h2 class="text-xl font-display font-bold">Buat Pengajuan Barang</h2>
                <button onclick="closeRequestModal()" class="text-white/70 hover:text-white cursor-pointer"><i data-lucide="x" class="w-6 h-6"></i></button>
            </div>
            <form action="buat_pengajuan.php" method="POST" class="p-8 space-y-5 bg-white">
                <div class="space-y-1">
                    <label class="text-xs font-bold text-luxury-primary uppercase ml-1">Pilih Barang Rusak</label>
                    <select name="item_id" required class="input-luxury w-full py-3 px-4 rounded-xl text-sm">
                        <?php 
                        mysqli_data_seek($result_inventory, 0);
                        while($item = mysqli_fetch_assoc($result_inventory)): ?>
                            <option value="<?= $item['id'] ?>"><?= htmlspecialchars($item['nama_barang']) ?> (<?= $item['kategori'] ?>)</option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-bold text-luxury-primary uppercase ml-1">Jumlah Penggantian</label>
                    <input type="number" name="qty" required min="1" class="input-luxury w-full py-3 px-4 rounded-xl text-sm" placeholder="Contoh: 2">
                </div>
                <div class="space-y-1">
                    <label class="text-xs font-bold text-luxury-primary uppercase ml-1">Alasan Penggantian</label>
                    <textarea name="alasan" required rows="3" class="input-luxury w-full py-3 px-4 rounded-xl text-sm" placeholder="Jelaskan kondisi barang saat ini..."></textarea>
                </div>
                <button type="submit" class="w-full py-4 bg-luxury-primary hover:bg-luxury-secondary text-white font-bold rounded-xl shadow-lg mt-4 transition-all cursor-pointer">Ajukan Sekarang</button>
            </form>
        </div>
    </div>
    ...
    <main class="flex-1 lg:ml-72 w-full min-h-screen flex flex-col">
            ...
            <div class="flex items-center gap-4">
                <div class="flex items-center gap-3 ml-2 pl-4 md:border-l border-slate-200 relative">
                    <div class="hidden md:block text-right">
                        ...
                        <p class="text-xs font-bold text-luxury-primary uppercase tracking-widest">Staff Fakultas</p>
                    </div>
                    <button onclick="toggleProfileDropdown()" class="w-10 h-10 rounded-full bg-indigo-50 overflow-hidden border-2 border-white shadow-sm hover:ring-2 hover:ring-luxury-primary/50 transition-all focus:outline-none cursor-pointer">
                        <img src="<?= isset($_SESSION['foto_profil']) && $_SESSION['foto_profil'] != 'default.png' ? '../uploads/profiles/' . $_SESSION['foto_profil'] : 'https://ui-avatars.com/api/?name=' . urlencode($nama_user) . '&background=4f46e5&color=fff' ?>" alt="Staff" class="w-full h-full object-cover">
                    </button>
                    <!-- Dropdown Menu -->
                    <div id="profile-dropdown" class="hidden absolute right-0 top-12 mt-2 w-48 bg-white border border-slate-100 rounded-2xl shadow-xl py-2 z-50">
                        <div class="px-4 py-3 border-b border-slate-50/80 mb-1 lg:hidden">
                            ...
                            <p class="text-xs text-slate-400 capitalize truncate">Staff Fakultas</p>
                        </div>
        ...
        <div id="admin-content-dashboard" class="p-6 md:p-10 max-w-7xl mx-auto w-full animate-popup">
            ...
            <div class="bg-gradient-to-tr from-luxury-primary to-luxury-secondary rounded-[2.5rem] p-8 md:p-10 text-white mb-8 relative overflow-hidden shadow-xl">
               <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                   <div>
                       ...
                       <p class="text-white/80 text-sm md:text-base max-w-xl leading-relaxed">Selamat bertugas di Sistem Pengelolaan Inventaris dan Peminjaman Fasilitas Akademik Universitas Muhammadiyah Sorong. Ada <b class="text-luxury-secondary bg-white px-2 py-0.5 rounded-md font-bold mx-1"><?= $count_pending ?> pengajuan</b> peminjaman baru yang perlu Anda proses.</p>
                   </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <button onclick="switchAdminTab('persetujuan')"
                    class="text-left bg-luxury-secondary rounded-[2rem] p-8 text-white shadow-soft relative overflow-hidden group hover:scale-[1.02] transition-transform">
                    <div class="relative z-10">
                        <p class="text-xs font-bold opacity-60 uppercase tracking-widest">Perlu Tindakan</p>
                        <h2 class="text-5xl font-display font-bold mt-2"><?= $count_pending ?></h2>
                    </div>
                    <i data-lucide="inbox"
                        class="absolute -bottom-4 -right-4 w-32 h-32 text-white/10 group-hover:scale-110 transition-transform duration-500"></i>
                </button>
        ...
        <div id="admin-content-persetujuan" class="hidden-section p-6 md:p-10 max-w-7xl mx-auto w-full animate-popup">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <h1 class="text-2xl font-display font-bold text-luxury-secondary">Verifikasi Pengajuan</h1>
                <div class="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full md:w-auto">
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-soft p-8">
                                ...
                                <?php while ($row = mysqli_fetch_assoc($result_pending)): ?>
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-right flex justify-end gap-2">
                                            <?php if ($row['status'] == 'return_request'): ?>
                                                <button
                                                    onclick="confirmAction('proses_persetujuan.php?id=<?= $row['id'] ?>&status=returned', 'Terima Pengembalian?', 'Stok akan bertambah otomatis.')"
                                                    class="px-3 py-2 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors font-bold text-xs flex items-center gap-1"><i
                                                        data-lucide="check-square" class="w-3 h-3"></i> Terima Kembali</button>
                                            <?php else: ?>
                                                <button
                                                    onclick="confirmAction('proses_persetujuan.php?id=<?= $row['id'] ?>&status=approved', 'Setujui Peminjaman?', 'Stok akan berkurang otomatis.')"
                                                    class="p-2 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition-colors"><i
                                                        data-lucide="check" class="w-4 h-4"></i></button>
                                                <button
                                                    onclick="confirmAction('proses_persetujuan.php?id=<?= $row['id'] ?>&status=rejected', 'Tolak Peminjaman?', 'Tindakan ini tidak dapat dibatalkan.', 'danger')"
                                                    class="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors"><i
                ...
                <?php else: ?>
                    <div class="flex flex-col items-center justify-center text-center">
                        ...
                        <p class="text-slate-500 font-medium">Semua tugas staff selesai.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                ...
                <div class="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full md:w-auto">
                    ...
                    <button onclick="openInventoryModal()"
                        class="px-6 py-3 bg-luxury-primary text-white font-bold rounded-xl shadow-lg hover:bg-luxury-secondary transition-all flex items-center justify-center gap-2 w-full md:w-auto">
                        ...
                        <tbody class="divide-y divide-slate-50" id="inventory-list">
                            ...
                            while ($item = mysqli_fetch_assoc($result_inventory)): ?>
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-right flex justify-end gap-2">
                                        <button
                                            onclick="editItem('<?= $item['id'] ?>', '<?= htmlspecialchars(addslashes($item['nama_barang'])) ?>', '<?= $item['kategori'] ?>', '<?= $item['stok'] ?>')"
                                            class="p-2 text-luxury-primary hover:bg-luxury-base rounded-lg transition-colors"><i
                                                data-lucide="edit-3" class="w-4 h-4"></i></button>
                                        <button
                                            onclick="confirmAction('hapus_barang.php?id=<?= $item['id'] ?>', 'Hapus Barang?', 'Data barang akan dihapus permanen.', 'danger')"
                                            class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><i
                        ...
                        <tbody class="divide-y divide-slate-50" id="table-users">
                            <?php while ($user = mysqli_fetch_assoc($result_users)): ?>
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-right flex justify-end gap-2">
                                        <button
                                            onclick="editUser('<?= $user['id'] ?>', '<?= htmlspecialchars($user['nama']) ?>', '<?= htmlspecialchars($user['username']) ?>', '<?= $user['role'] ?>', '<?= htmlspecialchars($user['fakultas']) ?>', '<?= htmlspecialchars($user['prodi']) ?>')"
                                            class="p-2 text-luxury-primary hover:bg-luxury-base rounded-lg transition-colors"><i
                                                data-lucide="edit-3" class="w-4 h-4"></i></button>
                                        <?php if ($user['id'] != $_SESSION['user_id']): ?>
                                            <button
                                                onclick="confirmAction('hapus_user.php?id=<?= $user['id'] ?>', 'Hapus User?', 'Akun user akan dihapus permanen.', 'danger')"
                                                class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors"><i
                                                    data-lucide="trash-2" class="w-4 h-4"></i></button>
                                        <?php endif; ?>
                                    </td>
        ...
        <div id="admin-content-pengajuan" class="hidden-section p-6 md:p-10 max-w-7xl mx-auto w-full animate-popup">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                <h1 class="text-2xl font-display font-bold text-luxury-secondary">Pengajuan Barang Rusak</h1>
                <div class="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full md:w-auto">
                    ...
                    <button onclick="openRequestModal()" class="px-6 py-3 bg-luxury-primary text-white font-bold rounded-xl shadow-lg hover:bg-luxury-secondary transition-all flex items-center justify-center gap-2 w-full md:w-auto">
                        <i data-lucide="plus" class="w-4 h-4"></i> Buat Pengajuan
                    </button>
                </div>
            </div>
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-soft p-8">
                <?php if (mysqli_num_rows($result_my_requests) > 0): ?>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left min-w-[700px]">
                            <thead class="bg-slate-50">
                                <tr>
                                    <th class="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest rounded-l-xl">Tanggal</th>
                            ...
                            <tbody class="divide-y divide-slate-50" id="table-pengajuan">
                                <?php while ($req = mysqli_fetch_assoc($result_my_requests)): ?>
                                    <tr>
                                        <td class="px-6 py-4 text-sm text-slate-500 font-medium"><?= date('d/m/Y H:i', strtotime($req['created_at'])) ?></td>
                                        <td class="px-6 py-4 font-bold text-slate-700">
                                            <?= htmlspecialchars($req['nama_barang']) ?>
                                            <span class="text-xs bg-slate-100 px-2 py-0.5 rounded-md text-slate-600 ml-2"><?= $req['qty'] ?> Unit</span>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-slate-500"><?= htmlspecialchars($req['alasan']) ?></td>
                                        <td class="px-6 py-4">
                                            <?php if ($req['status'] == 'pending_kaprodi'): ?>
                                                <span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full text-xs font-bold">Menunggu Kaprodi</span>
                                            <?php elseif ($req['status'] == 'approved_kaprodi'): ?>
                                                <span class="bg-blue-100 text-blue-700 px-2 py-1 rounded-full text-xs font-bold">Menunggu Dekan</span>
                                            <?php elseif ($req['status'] == 'approved_dekan'): ?>
                                                <span class="bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full text-xs font-bold">Disetujui</span>
                                            <?php else: ?>
                ...
                <?php else: ?>
                    <div class="flex flex-col items-center justify-center text-center">
                        <i data-lucide="file-edit" class="w-12 h-12 text-slate-200 mb-4"></i>
                        <h3 class="text-lg font-bold text-slate-700 mb-1">Belum ada pengajuan</h3>
                        <p class="text-slate-500 text-sm">Anda belum membuat pengajuan penggantian barang rusak.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
...
<script>
    ...
    function editItem(id, nama, kategori, stok) {
